...
# -- --------------------------------------------------------------------------- Cargar datos Precios M1 -- #
# leer archivo de precios historicos ya descargados previamente
df_pe_m1 = pd.read_csv("archivos/Eur_Usd_M1.csv")
df_pe_m1['timestamp'] = pd.to_datetime(df_pe_m1['timestamp']).dt.floor('Min')
df_pe_m1['timestamp'] = df_pe_m1['timestamp'].dt.tz_localize('UTC')

# -- --------------------------------------------------------------------------- Cargar datos Precios M5 -- #
# df_pe_m5 = pd.read_csv("archivos/Eur_Usd_M5.csv")
# df_pe_m5['timestamp'] = pd.to_datetime(df_pe_m5['timestamp']).dt.floor('Min')
# df_pe_m5['timestamp'] = df_pe_m5['timestamp'].dt.tz_localize('UTC')

# -- ---------------------------------------------------------------------------- Cargar datos Precios W -- #
df_pe_w = pd.read_csv("archivos/Eur_Usd_W.csv")
df_pe_w['timestamp'] = pd.to_datetime(df_pe_w['timestamp']).dt.floor('Min')
df_pe_w['timestamp'] = df_pe_w['timestamp'].dt.tz_localize('UTC')
...
# eliminar datos de indicadores en lista de negra (no habia precios o fechas estaban dificiles)
...
# df_pe.to_csv(r"archivos/Eur_Usd_W.csv", index=False)
